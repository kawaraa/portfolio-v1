...
function typeText(str, parent, time) {
  Array.from(str).forEach(el => setTimeout(() => parent.innerHTML += el, time += 50));
  return time;
}
function typeIntroduction() {
  ...
  let time1 = typeText("Feel free to take a look at my work on ", span1, time);
  let a2 = util.createAppend("a", introduction, { href: "#portfolio", class: "gen-links" });
  ...
  let a4 = util.createAppend("a", introduction, { href: "mailto:info@kawaraa.com?Subject=Hello%20again", class: "gen-links" });
...
function showHidList() {
  ...
  if (menuIcon.className === "m-icon") {
    menuIcon.className = "x-icon"; dropList.style.top = "45px";
    listLinks.forEach(el => el.style.animation = "margin-max 1s ease 0.5 forwards");
  } else {
    menuIcon.className = "m-icon"; dropList.style.top = "-200px";
...
function animateLinks() {
  ...
  setTimeout(() => navLinks.forEach(el => {
...
document.getElementById("contact-form").addEventListener("submit", (e) => {
  ...
  util.postJSON("contact", form).then(res => {
