...
const requestAnimation = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame;
...
onload = () => {
  ...
  document.querySelectorAll(".pages").forEach(el => { el.style.width = width + "px" });
  ...
  function multiplyObject(canvas, num, size, spd) {
    ...
    const shapes = [];
    for (let i = 0; i < num; i++) {
      shapes.push(new Circle(ctx, width / 2 + 50, height / 2 + (i * 2), i, size, "#D9005A", spd, width / 2 + (i * i)));
